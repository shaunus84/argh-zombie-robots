package entities {
	...
	public class Entity extends Sprite implements IEntity {
		...
		protected var _boundingQuad : Quad;

		public function create(spriteName : String, startAnim : String, startPos : Point, scaling : Number, moveSpeed : Number) : void {
			...
			Starling.current.stage.addChild(_spriterClip);
			Starling.juggler.add(_spriterClip);

			_boundingQuad = new Quad(_spriterClip.width, _spriterClip.height, 0xff0000);
			_boundingQuad.x -= _spriterClip.width / 2;
			_boundingQuad.y -= _spriterClip.height / 2;
			_spriterClip.addChild(_boundingQuad);
		...
		public function handleFrameListeners() : void {
			...
			_spriterClip.animationComplete.add(function(clip : SpriterClip) : void {
		...
		public function getBoundingRect() : Rectangle {
			return new Rectangle(_spriterClip.x, _spriterClip.y, _spriterClip.width / AssetLoader._scaling, _spriterClip.height / AssetLoader._scaling);
		}
